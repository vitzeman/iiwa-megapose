...
def generate_board_full(
    tag_size_mm: int,
    dpi: int,
    paper_size: Union[str, Tuple[int, int]] = "A4",
    margins: int = 10,
    tag_family: str = "DICT_APRILTAG_36h11",
) -> (cv2.aruco.Board, Tuple[int, int], dict):
    # TODO: add docstring
    if isinstance(paper_size, str):
        if paper_size not in SIZES.keys():
            raise ValueError(
                f"Paper size {paper_size} not supported yet.\n Supported sizes: {SIZES.keys()}"
            )
        paper_size = SIZES[paper_size]
        ...
        raise ValueError("paper_size must be either str or tuple")
    tag_size_px = int(tag_size_mm * dpi / 25.4)
    margins_px = margins
...
def create_full_board(
    ...
    paper_size: Union[str, Tuple[int, int]] = "A4",
    margins: int = 10,
    output_file: str = "data/page",
) -> None:
    board, res, points_dict = generate_board_full(tag_size_mm, dpi, paper_size, margins)
    with open(output_file + ".json", "w") as f:
        json.dump(points_dict, f, indent=2)
    img = board.generateImage(res, marginSize=margins, borderBits=1)
    cv2.imwrite(output_file + ".png", img)
    print(output_file + ".png")
    # convert to pdf
    # TODO
    ...
    with open("name.pdf", "wb") as f:
        f.write(img2pdf.convert(output_file + ".png", layout_fun=layout_fun))
...
if __name__ == "__main__":
    ...
    create_full_board(tag_size, dpi, "A4", edges_space_px, "data/april_full2")
