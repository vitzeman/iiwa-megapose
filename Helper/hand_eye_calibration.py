...
def get_RT(frame, camera_matrix, dist_coeff, board, distortion_coefficients):
    ...
    ret, p_rvec, p_tvec = cv2.aruco.estimatePoseCharucoBoard(
        c_corners, c_ids, board, camera_matrix, dist_coeff, np.empty(1), np.empty(1)
    )
...
def print_things(key, angle_marker, Tvec, angle_flange, tran_flange):
    ...
    print(
        math.degrees(angle_flange[0]) % 360,
        math.degrees(angle_flange[1]) % 360,
        math.degrees(angle_flange[2]) % 360,
    )
    print(tran_flange)
...
def main():
    ...
    camera_matrix = np.array([[2937, 0, 1224], [0, 2937, 1024], [0, 0, 1]])
